...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.combined_output_projection = nn.Linear(
            in_features=3 * self.hidden_size, out_features=self.hidden_size)
    ...
    def encode(
        ...
        pack_X = pack_padded_sequence(input=X,
                                      lengths=source_lengths,
                                      enforce_sorted=True)
        enc_hiddens = self.encoder(pack_X)
        last_hidden, last_cell = enc_hiddens[1]
        enc_hiddens, _ = pad_packed_sequence(sequence=enc_hiddens[0])
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        batch_size = X.shape[1]
        f_hidden = last_hidden[0].squeeze()  # (b, h)
        b_hidden = last_hidden[1].squeeze()  # (b, h)
        last_hidden = torch.cat((f_hidden, b_hidden), dim=1)
        init_decoder_hidden = self.h_projection(last_hidden)
        f_cell = last_cell[0].squeeze()
        b_cell = last_cell[1].squeeze()
        last_cell = torch.cat((f_cell, b_cell), dim=1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hiddens_proj = self.att_projection(
            enc_hiddens.reshape(-1, enc_hiddens.shape[-1])).view(
                batch_size, src_len, self.hidden_size)
        target_sents = self.model_embeddings.target(target_padded)

        for y_t in torch.split(target_sents, 1, dim=0):
            y_t = y_t.squeeze()
            Ybar_t = torch.cat((y_t, o_prev), dim=1)
            (dec_hidden,
             dec_cell), o_t, e_t = self.step(Ybar_t=Ybar_t,
                                             dec_state=dec_state,
    ...
    def step(
        ...
        dec_hidden = dec_hidden.unsqueeze(dim=-1)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden).squeeze()  # (b, src_len)
        ### END YOUR CODE
        ...
        ###
        attention_score = F.softmax(e_t, dim=1)
        attention_score = attention_score.unsqueeze(dim=1)
        a_t = torch.bmm(attention_score, enc_hiddens).squeeze()  # (b, 2*h)
        u_t = torch.cat((a_t, dec_hidden.squeeze()), dim=1)  # (b, 3*h)
