...
def minibatch_parse(sentences, model, batch_size):
    ...
    while not len(unfinished_partial_parses) == 0:
        transitions = model.predict(unfinished_partial_parses[:batch_size])
        for parser, transition in zip(unfinished_partial_parses[:batch_size], transitions):
            parser.parse_step(transition)
            if len(parser.buffer) == 0 and len(parser.stack) == 1:
                unfinished_partial_parses.remove(parser)
