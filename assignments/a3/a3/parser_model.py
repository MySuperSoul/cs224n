...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(in_features=self.embed_size * self.n_features, out_features=self.hidden_size)
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits = nn.Linear(in_features=self.hidden_size, out_features=self.n_classes)
    ...
    def forward(self, t):
        ...
        hiddens = self.embed_to_hidden(embeddings)
        hiddens = nn.functional.relu(hiddens)
